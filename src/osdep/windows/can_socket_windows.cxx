...
can_ops_t can_socket_ops = {
	...
	can_socket_send,
	can_socket_recv,
	can_socket_bitrate_set,
	can_socket_start,
	can_socket_stop,
	can_socket_state_get,
	can_socket_restart
};


int can_socket_create(const char *dev, unsigned bitrate)
{
	char cntstr[128];
	int handle;
...
int can_socket_destroy(int fd)
{
	return CanalClose(fd);
}

int can_socket_send(int fd, unsigned id, uint8_t dlc, void *data)
{
	canalMsg msg;
	int ret;
...
int can_socket_recv(int fd, unsigned *id, uint8_t *dlc, void *data,
    int64_t *sec, int64_t *usec)
{
	canalMsg msg;
	int ret;
...
int can_socket_bitrate_set(const char *, unsigned bitrate)
{
	int ret;
	int baudrate;
...
int can_socket_start(const char *)
{
	return 0;
}

int can_socket_stop(const char *)
{

	return 0;
}

int can_socket_state_get(const char *, qcan_state_t *status)
{
	int r;
	canalStatus stat;
...
int can_socket_restart(const char *)
{
	return 0;
}
